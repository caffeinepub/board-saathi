...
export default function TargetsPage() {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      toast.success(
        "Target added! You'll get an alarm 1 hour before the deadline.",
      );
      setTitle("");
      setDescription("");
      setDeadline("");
  ...
  const handleToggleComplete = async (
    ...
    try {
      await completeTargetMutation.mutateAsync({
        targetId,
        completed: !currentCompleted,
      });
      toast.success(
        currentCompleted ? "Target marked incomplete" : "Target completed! 🎉",
      );
    } catch {
      ...
      {targets.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-muted-foreground mb-4">
            Set study targets to keep yourself motivated
          </p>
          <Button onClick={() => setAddOpen(true)} className="gap-2">
            <Plus className="w-4 h-4" />
            Add Target
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-start gap-3">
                          <button
                            type="button"
                            onClick={() =>
                              handleToggleComplete(target.id, target.completed)
                            }
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-start justify-between gap-2">
                              ...
                              <div className="flex items-center gap-1 flex-shrink-0">
                                <Badge variant={dl.variant} className="text-xs">
                                  {dl.badge}
                                </Badge>
                                <Button
                                  variant="ghost"
                            ...
                            {alarmActive && (
                              <p className="text-xs text-amber-600 dark:text-amber-400 mt-1 flex items-center gap-1">
                                <AlarmClock className="w-3 h-3" />⏰ 1hr alarm
                                set
                              </p>
                            )}
                          </div>
            ...
            <div>
              ...
              <div className="space-y-3">
                {completed.map((target) => (
                  <Card
                    key={target.id}
                    className="opacity-60 border-l-4 border-l-green-500"
                  >
                    <CardContent className="p-4">
                      <div className="flex items-start gap-3">
                        <button
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              Set a study goal with a deadline. You'll get an alarm 1 hour
              before the deadline.
            </DialogDescription>
          </DialogHeader>
