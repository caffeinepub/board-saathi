...
function FlashCard({
  ...
  return (
    <div className="relative">
      <button
        type="button"
        className={`w-full text-left rounded-xl border-2 transition-all min-h-[160px] flex flex-col ${
          card.learned
...
export default function FlashcardsPage() {
  ...
  useEffect(() => {
    ...
    if (filterSubjectId !== "all") {
      filtered = filtered.filter(
        (f) => f.subjectId === Number.parseInt(filterSubjectId, 10),
      );
    }
    setCards(filtered);
  ...
  const chaptersForSelected = allChapters.filter((c) =>
    selectedSubjectId
      ? c.subjectId === Number.parseInt(selectedSubjectId, 10)
      : false,
  );
  ...
  const handleAdd = async () => {
    ...
    try {
      await addFlashcard.mutateAsync({
        chapterId: Number.parseInt(selectedChapterId, 10),
        subjectId: Number.parseInt(selectedSubjectId, 10),
        front: front.trim(),
        back: back.trim(),
      });
      ...
      {cards.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-muted-foreground mb-4">
            Create flashcards to boost your revision
          </p>
          <Button onClick={() => setAddOpen(true)} className="gap-2">
            <Plus className="w-4 h-4" />
            Add Flashcard
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              Create a new flashcard for quick revision.
            </DialogDescription>
          </DialogHeader>
